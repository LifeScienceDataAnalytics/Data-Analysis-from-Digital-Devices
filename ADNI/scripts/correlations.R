...
stats<-matrix(c('real',round(n_OP,2),'-',
                'decoded real',round(n_RP,2),round(relER_RP,2),
                'decoded virtual',round(n_VP,2),round(relER_VP,2)
)
,ncol=3,byrow=TRUE)
...
pl<-ggplot(data_corrs, aes(x=corr,fill=type))+geom_density(alpha=.2)+
  xlab('Correlation')+ylab('density')+ggtitle('ADNIwithDigital')+
  annotation_custom(tableGrob(stats), xmin=0.15, ymax=10)
pl
ggsave('output_figures/ADNI_normcorrs.png', pl, device = "png", width=7.26, height = 4.35)
ggsave('output_figures/ADNI_normcorrs.eps', pl, device = "eps", width=7.26, height = 4.35)
pl<-ggplot(data_corrs, aes(x=type,y=corr,fill=type))+
  geom_violin(position=position_dodge(1)) + 
  geom_boxplot(position=position_dodge(1),width=0.05) + 
  xlab('type')+ylab('density')+ggtitle('ADNIwithDigital')+coord_cartesian(ylim=c(-1,1))+
  annotation_custom(tableGrob(stats), xmin=1.5, ymax=-0.35)
pl
ggsave(paste0('output_figures/ADNI_normcorrs_violin.png'), pl, device = "png", width=7.26, height = 4.35)
ggsave(paste0('output_figures/ADNI_normcorrs_violin.eps'), pl, device = "eps", width=7.26, height = 4.35)
