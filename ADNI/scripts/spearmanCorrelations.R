...
#pearsonPadj$from = gsub("_VIS1", "", pearsonPadj$from)


spearmanPadj$to = gsub("zcode_", "", spearmanPadj$to)
spearmanPadj$to = gsub("SA_", "", spearmanPadj$to)
#pearsonPadj$to = gsub("_VIS1", "", pearsonPadj$to)
boot_stren_50_percent <- read_delim("boot.strenwithThreshold_all.csv",  ",", escape_double = FALSE, trim_ws = TRUE)

boot_stren_50_percent = merge(boot_stren_50_percent, spearmanPadj, all.x = TRUE)
boot_stren_50_percent$X1 = NULL
boot_stren_50_percent$X = NULL
write.csv(boot_stren_50_percent, "network_ADNI_r_CI.csv")
