...
def set_settings(f,opts):
    ...
    template = '--epochs 500 --model_name model_HIVAE_inputDropout --restore 0 --train 1         --data_file data_python_Notmissing/INPUT_FILE.csv --types_file data_python_Notmissing/TYPES_FILE          --batch_size NBATCH --save 499 --save_file INPUT_FILE        --dim_latent_s 1 --dim_latent_z 1 --dim_latent_y YDIM           --learning_rate LRATE --weight_decay WDECAY'
...
files=[i for i in os.listdir('data_python_Notmissing/') if not '_type' in i and '.DS_Store' not in i and not '_missing' in i and not 'stalone' in i]
...
def main():
    ...
    result_dataframe.to_csv("gridsearchresult_NotMissing/" + re.sub(r'.csv', '', f) + "_grid_search_results_NotMissing.csv", index=True)
