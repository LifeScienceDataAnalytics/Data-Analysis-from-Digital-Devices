...
def merge_dat(lis):
    ...
    for x in lis[1:]:
        ...
        ##change made by Meemansa for the new ALTOIDA version 4, as there are repetitive measurements for some patients
        ##can also try this##
...
def train_network(settings):
    ...
    train_data, types_dict, miss_mask, true_miss_mask, n_samples, old_data = read_functions.read_data(args.data_file, args.types_file, args.miss_file, args.true_miss_file)
    n_batches = int(np.floor(np.shape(train_data)[0]/args.batch_size))#Get an integer number of batches
    #print("n_batches train")
    #print(n_batches)
...
def enc_network(settings):
    ...
    with tf.Session(graph=sess_HVAE) as session:
        ...
        train_data_transformed = read_functions.discrete_variables_transformation(train_data, types_dict, old_data)
...
def dec_network(settings,zcodes,scodes,VP=False):
    ...
    #train_data, types_dict, miss_mask, true_miss_mask, n_samples = read_functions.read_data(args.data_file, args.types_file, args.miss_file, args.true_miss_file)
    train_data, types_dict, miss_mask, true_miss_mask, n_samples, old_data = read_functionsDig.read_data(args.data_file, args.types_file, args.miss_file, args.true_miss_file)
    
    print("Num")
    print(np.floor(np.shape(train_data)[0]))
    print("Den")
    print(args.batch_size)
    #Get an integer number of batches
    n_batches = int(np.floor(np.shape(train_data)[0]/args.batch_size))
    ...
    with tf.Session(graph=sess_HVAE) as session:
        ...
        for i in range(n_batches):      
            ...
            data_list, miss_list = read_functionsDig.next_batch(train_data, types_dict, miss_mask, args.batch_size, index_batch=i) #Create inputs for the feed_dict
            data_list_observed = [data_list[i]*np.reshape(miss_list[:,i],[args.batch_size,1]) for i in range(len(data_list))]#Delete not known data
            ...
            print(feedDict[tf_nodes['zcodes']])
        ...
        s_aux, z_aux, y_total, est_data = read_functionsDig.samples_concatenation(samples_list)
        ...
        est_data_transformed = read_functionsDig.discrete_variables_transformation(est_data, types_dict, old_data)
